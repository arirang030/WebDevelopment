        ...
        (function () {
            ...
            saveButtons.forEach(saveButton => {
                saveButton.addEventListener("click", function () {
                    if (!placesList.some(selectedPlace => selectedPlace === place)) {
                        // 마이로그에 장소 추가
                        getLogItem(place, placePosition);
                        // 선택한 장소에 마커 표시
                        changeMarker(placePosition, marker);
...
function getLogItem(places, position) {
    ...
    console.log(placesList[placesList.length - 1]);
    ...
    for (let delButton of delButtons) {
        delButton.addEventListener("click", function () {
            ...
            placesList = placesList.filter(place => place.address_name !== address);
...
// placesList에 있는 장소들의 경로 생성 함수
async function makeRoad(placesList) {
    ...
    // 도착지
    const destination = {
        x: placesList[placesList.length - 1].x,
        y: placesList[placesList.length - 1].y
    };

    // 중간 경유지
    const waypoints = placesList.slice(1, placesList.length - 1).map(place => {
        return {
            x: place.x,
            y: place.y
        };
...
function reset() {
    ...
    // 마이로그에 있는 장소 제거
    let logList = document.getElementById('mylogList');
    removeAllChildNods(logList);
